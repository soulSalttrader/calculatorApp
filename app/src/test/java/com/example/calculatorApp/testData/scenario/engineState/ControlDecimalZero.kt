...
object ControlDecimalZero : EngineState.Control {

    override val buildInput = { context: ContextEngineState -> buildControlInputState<ContextEngineState.Success>(context) }
    override val buildExpected = { context: ContextEngineState -> buildControlExpectedState<ContextEngineState.Success>(context) }
    ...
    override fun buildContexts(
        ...
        val expected = input.copy(
            expression = expressionInput,
            lastOperand = "${ButtonCalculatorNumber.Zero.symbol.label}.",
            lastOperator = button.toOperator(),
